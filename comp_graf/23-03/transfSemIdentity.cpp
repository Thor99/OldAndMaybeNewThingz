...
void display ()
{
    ...
    //glLoadIdentity(); // Como não carrega a identity, que resetava a transformacao, as matrizes vão se acumulando o tempo todo

    glMultMatrixf(mt); // Multiplica a matriz que já ta lá pelo parametro
    glMultMatrixf(mr); // Multiplica a matriz que já ta lá pelo parametro
    glMultMatrixf(ms); // Multiplica a matriz que já ta lá pelo parametro

    // O vertice irá multiplicar pela ordem contrária: escalar, rotação e depois translação
